...
\subsection{Análisis preliminar mediante Estadística, TDA y aprendizaje de variedades}
El primer acercamiento realizado sobre el dataset consistió en un análisis exploratorio utilizando herramientas estaditicas para ver la resolución e intensidad ene l dataset ya que al ser pequeño y tan heterogeno las técnicas de aprendizaje de variedades (\textit{manifold learning})
pueden ayudar.


El primer acercamiento realizado sobre el dataset consistió en un análisis exploratorio utilizando herramientas estadísticas y técnicas de aprendizaje de variedades (\textit{manifold learning}), con el objetivo de estudiar la distribución espacial de las imágenes, sus resoluciones, distribución de intensidades, balanceo entre clases y nivel de heterogeneidad presente en el conjunto de datos.



Para dicho análisis se utilizaron métricas estadísticas descriptivas, diagramas de caja (\textit{boxplots}), análisis de frecuencias, histogramas y distintos \textit{scores} asociados a estabilidad y distribución de las imágenes.

A partir de este análisis se observó que el dataset presentaba una alta variabilidad tanto en resolución espacial como en distribución de intensidades, lo cual representaba una dificultad importante para el modelado anatómico y el entrenamiento estable de redes neuronales convolucionales.

Adicionalmente, las proyecciones obtenidas mediante técnicas de \textit{manifold learning} sugirieron la existencia de estructuras geométricas y topológicas relevantes asociadas a la organización anatómica de la columna vertebral.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{images/Estadistica.png}
\caption{
Análisis estadístico exploratorio del conjunto de radiografías. La figura muestra las distribuciones del brillo global, las estadísticas de intensidad en la región de interés y la variabilidad del contraste a lo largo del dataset. Los histogramas evidencian una heterogeneidad significativa en las distribuciones de intensidad, mientras que el diagrama de dispersión ilustra la relación entre la intensidad media de la región de interés y el contraste local. Estas observaciones motivaron el uso de estrategias de normalización y técnicas de preprocesamiento para mejorar la consistencia estructural antes del entrenamiento de la CNN.
}
\label{fig:dataset_statistics}
\end{figure}

Mediante representaciones topológicas y proyecciones sobre espacios de baja dimensión, fue posible identificar patrones estructurales y agrupamientos asociados a distintas configuraciones anatómicas presentes en el dataset.

Sin embargo, posteriormente se realizaron estudios de ablación con el objetivo de evaluar el impacto real de estas representaciones dentro del proceso de segmentación.

Los resultados obtenidos indicaron que las representaciones topológicas y geométricas derivadas de TDA y \textit{manifold learning} contribuyen principalmente a:

\begin{itemize}
    \item mejorar la separabilidad entre representaciones,
    \item generar agrupamientos más estructurados,
    \item enriquecer la organización espacial del espacio latente,
    \item y proporcionar contexto geométrico adicional al proceso de aprendizaje.
\end{itemize}

No obstante, también se observó que dichas representaciones, por sí solas, no eran suficientes para resolver completamente el problema de segmentación anatómica.

A partir de estas observaciones se decidió integrar la información topológica y geométrica como un mecanismo complementario dentro de una arquitectura convolucional enriquecida, en lugar de utilizarla como única fuente de representación.
